#ifndef CRC_H
#define CRC_H

// #include <stdio.h>

uint64_t CRC4_encode (uint32_t dataword, uint8_t dwSize, uint8_t generator_polynomial_5bit)
{
    if(generator_polynomial_5bit == 0x0) generator_polynomial_5bit = 0x17;

    uint8_t crc_append = 0x0;
    uint8_t circuitbits = 0x0;

    for(int i = 0; i < (dwSize+4); i++)
    {
        // printf("0x%x | ", circuitbits);
        //shift
        circuitbits = (circuitbits << 1) & 0x1F;
        //append
        uint8_t nextbit = (dataword >> ((dwSize-1)-i)) & 0x1;
        circuitbits ^= nextbit;
        //look at first bit
        if(circuitbits & 0x10)
        {
            circuitbits ^= generator_polynomial_5bit;
        }
        // continue or xor with generator
        // printf("0x%x\n", circuitbits);
    }

    crc_append = circuitbits & 0x0F;

    return (((uint64_t)dataword) << 4) | crc_append;
}

uint8_t CRC4_decode (uint64_t dataword, uint8_t dwSize, uint8_t generator_polynomial_5bit)
{
    if(generator_polynomial_5bit == 0x0) generator_polynomial_5bit = 0x17;

    uint8_t crc_append = 0x0;
    uint8_t circuitbits = 0x0;

    for(int i = 0; i < dwSize; i++)
    {
        // printf("0x%x | ", circuitbits);
        //shift
        circuitbits = (circuitbits << 1) & 0x1F;
        //append
        uint8_t nextbit = (dataword >> ((dwSize-1)-i)) & 0x1;
        circuitbits ^= nextbit;
        //look at first bit
        if(circuitbits & 0x10)
        {
            circuitbits ^= generator_polynomial_5bit;
        }
        // continue or xor with generator
        // printf("0x%x\n", circuitbits);
    }

    // printf("circuit bits after calc: %x\n", circuitbits);
    crc_append = circuitbits & 0x0F;

    return crc_append;
}

#endif